...
Page({
    ...
    async loadPage() {
        ...
        try {
            ...
            const result = await (0, application_service_1.detail)(this.data.applicationId);
            const canView = result.permissionState.canViewPublisherContact;
            const withdrawBlockReason = (0, application_service_1.getWithdrawBlockReason)(result.application.status);
            const canWithdraw = !withdrawBlockReason;
            this.setData({
                pageState: page_state_1.PAGE_STATUS.ready,
                detail: result,
                statusText: (0, formatter_1.formatApplicationStatus)(result.application.status),
                noticeStatusText: result.noticeSummary.status ? (0, formatter_1.formatNoticeStatus)(result.noticeSummary.status) : '待同步',
    ...
    onPrimaryAction() {
        ...
        (0, router_1.navigateByRoute)(`/pages/plaza/notice-detail?noticeId=${this.data.detail.noticeSummary.noticeId}`);
    },
    async onSecondaryAction() {
        ...
        if (!this.data.canWithdraw) {
            (0, router_1.navigateByRoute)('/packages/creator/application-list/index');
            return;
        }
        ...
        try {
            await (0, application_service_1.withdraw)(this.data.detail.application.applicationId, {
