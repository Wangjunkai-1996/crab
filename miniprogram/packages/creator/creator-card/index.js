...
Page({
    ...
    applyDerivedState(partial = {}) {
        const cardForm = partial.cardForm || this.data.cardForm;
        ...
        this.setData({
            ...
            previewSummary: cardForm.primaryPlatform && cardForm.primaryCategory && cardForm.followerBand
                ? `${(0, formatter_1.formatPlatform)(cardForm.primaryPlatform)} · ${(0, formatter_1.formatFollowerBand)(cardForm.followerBand)} · ${(0, formatter_1.formatCategory)(cardForm.primaryCategory)}`
                : '先补齐平台、领域、粉丝量级和联系方式，发布方才能快速判断是否适合合作。',
            previewTags: [
                { label: cardForm.city?.trim() || '待补城市', active: !!cardForm.city?.trim() },
                { label: (0, formatter_1.formatGender)(cardForm.gender) || '性别未设置', active: !!cardForm.gender },
                { label: (0, formatter_1.formatContactType)(cardForm.contactType), active: !!cardForm.contactType },
    ...
    async loadPage() {
        ...
        try {
            ...
            const result = await (0, creator_service_1.getCard)();
            ...
            this.applyDerivedState({
                ...
                isFirstCreate,
                completenessText: isFirstCreate ? '待创建' : (0, formatter_1.formatCompleteness)(result.profileCompleteness),
    ...
    async onSave() {
        ...
        try {
            const result = await (0, creator_service_1.upsertCard)(this.data.cardForm);
            this.setData({
                fieldErrors: {},
                completenessText: (0, formatter_1.formatCompleteness)(result.profileCompleteness),
                isFirstCreate: false,
            });
            wx.showToast({
                title: '名片已保存',
