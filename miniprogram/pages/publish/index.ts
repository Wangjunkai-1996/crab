...
function buildProgressCards(profileForm: PublisherFormState, noticeForm: NoticeFormState): ProgressCard[] {
  const budgetFields = [
    noticeForm.city,
  ...
  const cards: Array<Omit<ProgressCard, 'statusText' | 'complete'>> = [
    {
      key: 'profile',
      title: '发布方资料',
...
function buildSectionProgress(progressCards: ProgressCard[]) {
  const findCard = (key: string) => progressCards.find((item) => item.key === key)?.statusText || '';
...
function buildNextStep(progressCards: ProgressCard[], profileIncomplete: boolean, draftNoticeId: string) {
