...
import type { DiscoveryFilter, NoticeDetail, NoticeDetailResponse } from '../models/notice';
import { formatNoticeStatus } from '../utils/formatter';
import type { CreatorCard, MineSummaryResponse, PublisherProfile, RoleFlags } from '../models/user';
...
let preferredView: 'publisher' | 'creator' = 'publisher';

let publisherProfile: PublisherProfile | null = {
...
let creatorCard: CreatorCard | null = {
...
type MockNotice = NoticeDetail & {
  ...
  ctaState: NoticeDetailResponse['ctaState'];
  maskedOrFullContact?: string;
  highlightTag?: string;
  applicationCount?: number;
};

const publicNoticeFixtures: MockNotice[] = [
  {
    noticeId: 'notice-001',
...
const myNoticeFixtures: MockNotice[] = [
  {
...
type MockPublisherApplication = PublisherApplicationListItem & {
  noticeId: string;
  creatorContact: string;
  creatorSummary: PublisherApplicationDetailResponse['creatorSummary'];
  application: PublisherApplicationDetailResponse['application'];
};

let publisherApplications: MockPublisherApplication[] = [
  {
...
function createResponse<T>(data: T, code = 0, message = 'ok'): ApiResponse<T> {
  requestSeed += 1;

  return {
    code,
    message,
    data: data as ApiResponse<T>['data'],
    requestId: `mock-${Date.now()}-${requestSeed}`,
  };
}

function buildFilterSummary(filter: DiscoveryFilter, total: number) {
  return {
    total,
...
function buildFilterEcho(filter: DiscoveryFilter) {
  return {
...
function buildContactInfo(contactText?: string) {
  if (!contactText) {
    return null;
  }
...
function toNoticeCardDto(item: MockNotice) {
  return {
    ...
    statusTag: {
      code: item.statusTag,
      label: formatNoticeStatus(item.statusTag),
    },
...
function buildMineSummary(): MineSummaryResponse {
...
function filterPublicNotices(payload: DiscoveryFilter) {
...
function findNoticeById(noticeId: string) {
...
function getPublisherApplicationActions(current: MockPublisherApplication): PublisherApplicationAction[] {
...
function findPublisherApplication(applicationId: string) {
  return publisherApplications.find((item) => item.applicationId === applicationId);
}

function updatePublisherApplication(applicationId: string, updater: (current: MockPublisherApplication) => MockPublisherApplication) {
  let nextItem: MockPublisherApplication | undefined;
...
function buildPublisherApplicationDetail(current?: MockPublisherApplication): PublisherApplicationDetailResponse {
  return {
...
function buildApplicationDetail(current?: ApplicationListItem): ApplicationDetailResponse {
  ...
  const timelineMap: Record<string, Array<{ label: string; time: string; description?: string }>> = {
    applied: [
      {
        label: '你已完成报名',
...
type Handler = (payload: Record<string, unknown>) => ApiResponse<any>;
...
const handlers: Record<string, Handler> = {
  ...
  'creator-bff:upsertCard': (payload) => {
    creatorCard = {
      ...
      portfolioImages: (payload.portfolioImages as string[] | undefined) || creatorCard?.portfolioImages || [],
  ...
  'notice-bff:list': (payload) => {
    const filter = payload as DiscoveryFilter;
  ...
  'notice-bff:updateDraft': (payload) =>
    createResponse({
      noticeId: payload.noticeId,
      status: 'draft',
      updatedAt: buildMockIsoTime(),
    }),
  'notice-bff:submitReview': (payload) =>
    createResponse({
      noticeId: payload.noticeId,
      status: 'pending_review',
      reviewRoundCount: 1,
  ...
  'notice-bff:close': (payload) =>
    createResponse({
      noticeId: payload.noticeId,
      status: 'closed',
      closedAt: buildMockIsoTime(),
    }),
  'notice-bff:republish': (payload) =>
    createResponse({
      noticeId: payload.noticeId,
      status: 'pending_review',
      reviewRoundCount: 2,
...
export async function mockCallFunction<T, P extends object = Record<string, unknown>>(name: string, data: CloudActionRequest<P>): Promise<ApiResponse<T>> {
  await sleep();
  ...
  return handler(data.payload as Record<string, unknown>) as ApiResponse<T>;
}
